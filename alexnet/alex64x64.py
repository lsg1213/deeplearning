...
dataset_validation = dataset_validation.batch(256)
...
def getModel():
    return keras.Sequential([
        keras.layers.Conv2D(96, input_shape=(64, 64, 3), kernel_size=(
            7, 7), strides=2, activation='relu'),  # (20,20)
        keras.layers.MaxPool2D(pool_size=(3, 3),strides=2),  # (18,18)
        keras.layers.BatchNormalization(),

        keras.layers.ZeroPadding2D(padding=(1, 1)),  # (19,19)
        keras.layers.Conv2D(256, (5,5), strides=1, activation='relu'),  # (15,15)
        keras.layers.MaxPool2D(pool_size=(3, 3), strides=2),  # (7,7)
        keras.layers.BatchNormalization(),

        keras.layers.ZeroPadding2D(padding=(1, 1)),  # (9,9)
        keras.layers.Conv2D(384, 3, strides=1, activation='relu'),  # (7,7)

        keras.layers.ZeroPadding2D(padding=(1, 1)),  # (9,9)
        keras.layers.Conv2D(384, 3, strides=1, activation='relu'),  # (7,7)

        keras.layers.ZeroPadding2D(padding=(1, 1)),  # (9,9)
        keras.layers.Conv2D(256, 3, strides=1, activation='relu'),  # (7,7)
